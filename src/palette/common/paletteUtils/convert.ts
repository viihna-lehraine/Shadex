...
function cmykToHSL(cmyk: CMYK): HSL {
	try {
		...
		if (mode.logErrors) console.error(`cmykToHSL() error: ${error}`);
...
function cmykToRGB(cmyk: CMYK): RGB {
	try {
		...
		if (mode.logErrors) console.error(`cmykToRGB error: ${error}`);
...
function hexToHSL(hex: Hex): HSL {
	try {
		...
		if (mode.logErrors) console.error(`hexToHSL() error: ${error}`);
			...
			typeof clonedInput === 'string'
				? {
						value: {
							...
							numAlpha: core.hexAlphaToNumericAlpha(
								clonedInput.slice(-2)
							)
						},
						format: 'hex'
...
function hexToRGB(hex: Hex): RGB {
	try {
		...
		if (mode.logErrors) console.error(`hexToRGB error: ${error}`);
...
function hslToHex(hsl: HSL): Hex {
	try {
		...
		if (mode.logErrors) console.error(`hslToHex error: ${error}`);
...
function hslToHSV(hsl: HSL): HSV {
	try {
		...
		if (mode.logErrors) console.error(`hslToHSV() error: ${error}`);
...
function hslToLAB(hsl: HSL): LAB {
	try {
		...
		if (mode.logErrors) console.error(`hslToLab() error: ${error}`);
...
function hslToRGB(hsl: HSL): RGB {
	try {
		...
		if (mode.logErrors) console.error(`hslToRGB error: ${error}`);
...
function hslToXYZ(hsl: HSL): XYZ {
	try {
		...
		if (mode.logErrors) console.error(`hslToXYZ error: ${error}`);
...
function hsvToHSL(hsv: HSV): HSL {
	try {
		...
		if (mode.logErrors) console.error(`hsvToHSL() error: ${error}`);
...
function labToHSL(lab: LAB): HSL {
	try {
		...
		if (mode.logErrors) console.error(`labToHSL() error: ${error}`);
...
function labToRGB(lab: LAB): RGB {
	try {
		...
		if (mode.logErrors) console.error(`labToRGB error: ${error}`);
...
function labToXYZ(lab: LAB): XYZ {
	try {
		...
		if (mode.logErrors) console.error(`labToXYZ error: ${error}`);
...
function rgbToCMYK(rgb: RGB): CMYK {
	try {
		...
		const key = core.sanitizePercentage(
			1 - Math.max(redPrime, greenPrime, bluePrime)
		);
		const cyan = core.sanitizePercentage(
			(1 - redPrime - key) / (1 - key) || 0
		);
		const magenta = core.sanitizePercentage(
			(1 - greenPrime - key) / (1 - key) || 0
		);
		const yellow = core.sanitizePercentage(
...
function rgbToHex(rgb: RGB): Hex {
	try {
		...
		if (mode.logErrors) console.warn(`rgbToHex error: ${error}`);
...
function rgbToHSL(rgb: RGB): HSL {
	try {
		...
		if (mode.logErrors) console.error(`rgbToHSL() error: ${error}`);
...
function rgbToHSV(rgb: RGB): HSV {
	try {
		...
		if (mode.logErrors) console.error(`rgbToHSV() error: ${error}`);
...
function rgbToXYZ(rgb: RGB): XYZ {
	try {
		...
		if (mode.logErrors) console.error(`rgbToXYZ error: ${error}`);
...
function xyzToLAB(xyz: XYZ): LAB {
	try {
		...
		if (mode.logErrors) console.error(`xyzToLab() error: ${error}`);
...
function xyzToHSL(xyz: XYZ): HSL {
	try {
		...
		if (mode.logErrors) console.error(`xyzToHSL() error: ${error}`);
...
function xyzToRGB(xyz: XYZ): RGB {
	try {
		...
		if (mode.logErrors) console.error(`xyzToRGB error: ${error}`);
...
function toHSL(color: Exclude<Color, SL | SV>): HSL {
	try {
		...
		switch (color.format) {
			...
			case 'hsl':
				return core.clone(clonedColor as HSL);
			case 'hsv':
