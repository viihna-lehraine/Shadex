...
const dbPromise: Promise<PaletteDB> = openDB<PaletteSchema>(
	'paletteDatabase',
	1,
	{
		upgrade(db) {
...
async function getDB(): Promise<PaletteDB> {
	return dbPromise;
}
...
async function getCustomColor(): Promise<HSL | null> {
	const db = await getDB();
...
async function getTable(id: string): Promise<StoredPalette | null> {
	const db = await getDB();
...
async function getStore<StoreName extends keyof PaletteSchema>(
	storeName: StoreName,
	mode: 'readonly'
): Promise<IDBPObjectStore<PaletteSchema, [StoreName], StoreName, 'readonly'>>;

async function getStore<StoreName extends keyof PaletteSchema>(
	storeName: StoreName,
	mode: 'readwrite'
): Promise<IDBPObjectStore<PaletteSchema, [StoreName], StoreName, 'readwrite'>>;
...
async function logMutation(log: MutationLog): Promise<void> {
	const db = await getDB();
...
async function trackedTransaction<StoreName extends keyof PaletteSchema>(
	storeName: StoreName,
	mode: 'readonly' | 'readwrite',
	callback: (
