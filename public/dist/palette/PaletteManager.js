...
class PaletteManager {
    ...
    constructor(stateManager, common, generateHues, generatePaletteFns, generatePalette) {
        ...
        this.log('debug', 'Completing initialization', 'PaletteManager.constructor()', 2);
    }
    extractPaletteFromDOM() {
        return this.errors.handle(() => {
            this.log('debug', 'Extracting palette from DOM', 'PaletteManager.extractPaletteFromDOM()', 2);
            const paletteColumns = document.querySelectorAll('.palette-column');
            if (paletteColumns.length === 0) {
                this.log('debug', 'No palette columns found in DOM.', 'PaletteManager.extractPaletteFromDOM()', 2);
            ...
            const extractedItems = Array.from(paletteColumns).map((col, index) => {
                ...
                this.log('debug', `Extracted color from column ${index + 1}: ${color}`, 'PaletteManager.extractPaletteFromDOM()', 2);
                if (!data.config.regex.dom.hex.test(color)) {
                    this.log('warn', `Invalid color format for column ${index + 1}: ${color}`, 'PaletteManager.extractPaletteFromDOM()');
            ...
            this.log('debug', `Extracted palette options: ${JSON.stringify(options)}`, 'PaletteManager.extractPaletteFromDOM()', 2);
            return {
        ...
        }, 'Failed to extract palette from DOM', 'PaletteManager.extractPaletteFromDOM()');
    }
    handleColumnLock(columnID) {
        ...
        if (columnIndex === -1) {
            this.log('warn', `Column with ID ${columnID} not found.`, 'PaletteManager.handleColumnLock()');
            return;
        }
        // toggle lock state
    ...
    handleColumnResize(columnID, newSize) {
        this.errors.handle(() => {
            ...
            const columnIndex = columns.findIndex(col => col.id === columnID);
            if (columnIndex === -1) {
                this.log('warn', `Column with ID ${columnID} not found.`, 'PaletteManager.handleColumnResize()');
                return;
            }
            ...
            this.stateManager.updatePaletteColumns(columns, true, 2);
        }, `Failed to resize column ${columnID}`, 'PaletteManager.handleColumnResize()');
    }
    async loadPalette() {
        await this.errors.handleAsync(async () => {
            ...
            if (storedPalette &&
                ...
                this.log('info', `Stored palette added to history`, 'PaletteManager.loadPalette()', 2);
            }
            const randomOptions = this.utils.palette.getRandomizedPaleteOptions();
            this.log('debug', `Generated randomized palette options: ${JSON.stringify(randomOptions)}`, 'PaletteManager.loadPalette()', 2);
            ...
            this.log('info', `New palette added to history`, 'PaletteManager.loadPalette()', 2);
            await this.storage.setItem('palette', newPalette);
            this.log('info', `New palette stored`, 'PaletteManager.loadPalette()', 2);
            this.renderPaletteFromState();
            this.log('info', `Palette rendered from state`, 'PaletteManager.loadPalette()', 2);
        }, 'Failed to load palette', 'PaletteManager.loadPalette()');
    }
    async renderNewPalette() {
        ...
        }, 'Failed to render a new palette', 'PaletteManager.renderNewPalette()');
    }
    async renderPaletteFromState() {
        await this.errors.handleAsync(async () => {
            ...
            const paletteContainer = this.utils.core.getElement(ids.divs.paletteContainer);
            if (!paletteContainer) {
                this.log('error', 'Palette container not found', 'PaletteManager.renderPaletteFromState()', 1);
                return;
            }
            ...
            if (!latestPalette) {
                this.log('warn', 'No saved palettes in history. Cannot render.', 'PaletteManager.renderPaletteFromState()', 2);
                return;
            }
            ...
            this.log('info', `Restored ${columns.length} columns from saved state.`, 'PaletteManager.renderPaletteFromState()', 2);
        }, 'Failed to render palette from state', 'PaletteManager.renderPaletteFromState()');
    }
    swapColumns(draggedID, targetID) {
        this.errors.handle(() => {
            ...
            if (draggedIndex === -1 || targetIndex === -1) {
                this.log('warn', `Failed to swap: Column ID ${draggedID} or ${targetID} not found.`, 'PaletteManager.swapColumns()');
                return;
            }
            ...
            this.log('debug', `Swapped columns ${draggedID} and ${targetID}. New order: ${columns.map(col => col.id).join(', ')}`, 'PaletteManager.swapColumns()');
        }, `Failed to swap columns ${draggedID} and ${targetID}`, 'PaletteManager.swapColumns()');
    }
    createColumnInUI(column, columnID, colorValue) {
        this.errors.handle(() => {
            ...
            if (!this.utils.validate.userColorInput(colorValue)) {
                this.log('warn', `Invalid color value: ${colorValue}. Unable to render column UI.`, 'PaletteManager.createColumnInUI()');
                return;
            }
            const validColorSpace = ['hex', 'hsl', 'rgb'].includes(colorSpace)
                ? colorSpace
        ...
        }, 'Failed to create column UI elements', 'PaletteManager.createColumnInUI()', { columnID, colorValue });
    }
}
