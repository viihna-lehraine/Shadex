...
class StateManager {
    ...
    async init() {
        this.log('debug', 'Initializing State Manager', 'StateManager.init()', 2);
        await this.storage.init();
        this.state =
            (await this.errors.handleAsync(() => this.loadState(), 'Failed to load state. Generating initial state.', 'StateManager.init()')) ?? this.generateInitialState();
        this.log('info', 'StateManager initialized successfully.', 'StateManager.init()', 2);
    ...
    addPaletteToHistory(palette) {
        ...
        }, 'Failed to add palette to history', 'StateManager.addPaletteToHistory()', { palette });
    }
    async ensureStateReady() {
        ...
        while (!this.state || !this.state.paletteContainer) {
            if (attempts++ >= maxAttempts) {
                this.log('error', 'State initialization timed out.', 'StateManager.ensureStateReady()');
                break;
            }
            this.log('warn', `Waiting for state to initialize... (Attempt ${attempts})`, 'StateManager.ensureStateReady()');
            await new Promise(resolve => setTimeout(resolve, 100));
        }
        this.log('info', 'State is now initialized.', 'StateManager.ensureStateReady()');
    }
    getState() {
        return (this.errors.handle(() => {
            ...
            if (!this.state.preferences) {
                this.log('warn', 'State.preferences is undefined. Adding default preferences.', 'StateManager.getState()');
                this.state.preferences = defaultState.preferences;
            }
            return this.state;
        }, 'Error retrieving state', 'StateManager.getState()', {}, 'error') ?? defaultState);
    }
    async loadState() {
        ...
        if (storedState) {
            ...
            this.log('warn', 'No stored state found.', 'StateManager.loadState()', 3);
    ...
    redo() {
        ...
        }, 'Redo operation failed', 'StateManager.redo()');
    }
    async resetState() {
        ...
        }, 'Failed to reset state', 'StateManager.resetState()');
    }
    setOnStateLoad(callback) {
        this.errors.handle(() => {
            this.onStateLoadCallback = callback;
        }, 'Failed to set onStateLoad callback', 'StateManager.setOnStateLoad()');
    }
    ...
    undo() {
        this.errors.handle(() => {
            ...
            this.log('info', 'Undo action performed.', 'StateManager.undo()', 3);
            this.saveStateAndLog('undo', 3);
        }, 'Undo operation failed', 'StateManager.undo()');
    }
    updateAppModeState(appMode, track) {
        ...
        }, 'Failed to update app mode state', 'StateManager.updateAppModeState()', { appMode, track });
    }
    updatePaletteColumns(columns, track, verbosity) {
        ...
        }, 'Failed to update palette columns', 'StateManager.updatePaletteColumns()', { columns, track, verbosity });
    }
    updatePaletteColumnSize(columnID, newSize) {
        ...
        }, 'Failed to update palette column size', 'StateManager.updatePaletteColumnSize()', { columnID, newSize });
    }
    updatePaletteHistory(updatedHistory) {
        ...
        }, 'Failed to update palette history', 'StateManager.updatePaletteHistory()', { updatedHistory });
    }
    updateSelections(selections, track) {
        ...
        }, 'Failed to update selections', 'StateManager.updateSelections()', { selections, track });
    }
    generateInitialState() {
        ...
        }, 'Failed to generate initial state', 'StateManager.generateInitialState()') ?? defaultState);
    }
    ...
    async saveState() {
        await this.errors.handleAsync(() => this.storage.setItem('appState', this.state), 'Failed to save app state.', 'StateManager.saveState()');
    }
    trackAction() {
