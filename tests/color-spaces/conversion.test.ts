import {
	...
	genAllColorValues,
	conversionMap
} from '../../src/color-spaces/conversion';
...
describe('Color Conversion Utilities', () => {
	describe('getConversionFn', () => {
		...
		it('should return undefined if no conversion exists', () => {
			const conversionFn = getConversionFn(
				'xyz',
				'nonexistent' as keyof colors.ColorDataAssertion
			);
	...
	describe('genAllColorValues', () => {
		it('should generate all color values for a valid hex color', () => {
			...
			expect(allValues.rgb).toEqual({
				format: 'rgb',
				value: { red: 255, green: 0, blue: 0 }
			});
			expect(allValues.hsl).toEqual({
				format: 'hsl',
		...
		it('should generate all color values for a valid RGB color', () => {
			...
			expect(allValues.hsl?.value).toEqual({
				hue: 240,
				saturation: 100,
				lightness: 50
			});
