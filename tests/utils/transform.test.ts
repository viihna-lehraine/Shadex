...
describe('transform module', () => {
	...
	describe('colorToColorString', () => {
		...
		it('should throw an error for unsupported formats', () => {
			...
			expect(() => transform.colorToColorString(invalidColor)).toThrow(
				'Unsupported format'
			);
		});
	});
	...
	describe('colorStringToColor', () => {
		it('should convert HSL string back to HSL color', () => {
			const colorString: colors.HSLValueString = {
				hue: '180',
				saturation: '50%',
				lightness: '40%'
			};
			const result = transform.colorStringToColor({
				format: 'hsl',
				value: colorString
			});
	...
	describe('parseColor', () => {
		...
		it('should return null for unsupported color space', () => {
			const result = transform.parseColor(
				'unsupported' as any,
				'255,0,0'
			);
	...
	describe('parseCustomColor', () => {
		...
		it('should return null for unsupported format', () => {
			const result = transform.parseCustomColor(
				'unsupported' as any,
				'some value'
			);
			expect(result).toBeNull();
