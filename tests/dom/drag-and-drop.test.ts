...
describe('dragAndDrop Module', () => {
    ...
    beforeEach(() => {
        ...
        dropTargetElement = document.getElementById('drop-target') as HTMLElement;
    });
    ...
    describe('handleDragStart', () => {
        it('should set drag source element and initialize data transfer', () => {
            ...
            expect(event.dataTransfer?.getData('text/html')).toBe(dragSrcElement.outerHTML);
        });
    });
    ...
    describe('handleDrop', () => {
        it('should swap the HTML and values of drag source and drop target', () => {
            ...
            const attachListenersSpy = jest.spyOn(domHelpers, 'attachDragAndDropEventListeners');

            dragAndDrop.handleDragStart(createDragEvent('dragstart', dragSrcElement));
            dragAndDrop.handleDrop(event);

            const newDragSrcElement = document.getElementById('drag-source') as HTMLElement;
            const newDropTargetElement = document.getElementById('drop-target') as HTMLElement;

            expect(newDragSrcElement.querySelector('input')?.value).toBe('target color');
            expect(newDropTargetElement.querySelector('input')?.value).toBe('dragged color');
